...
plt.plot(f(torch.from_numpy(xx)).numpy(), d(torch.from_numpy(xx)).numpy()**(-1)*N(xx), 'b')
plt.plot(xx, N(xx), color='orange')
plt.legend(["f1(x)", "f0(x)"])
plt.show()
...
from assignment3.metric_estimator import objectiveJS, Discriminator
